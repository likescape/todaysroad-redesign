...
export default function SettingsScreen({ onBack }: { onBack: () => void }) {
  ...
  return <section className="settings-page page-surface">
    ...
    <div className="settings-content">{selected ? <div className="white-card settings-detail">{selected === "알림 설정" ? <><div className="notification-setting"><span>산책 리마인드</span><Toggle on={walk} onChange={setWalk} label="산책 리마인드" /></div><div className="notification-setting"><span>커뮤니티 알림</span><Toggle on={community} onChange={setCommunity} label="커뮤니티 알림" /></div><p>미리보기 설정이며 실제 알림은 발송되지 않아요.</p></> : <p>{selected === "앱 버전" ? "오늘의길 0.1.0" : `${selected} 기능은 준비 중이에요.`}</p>}</div> : <><AppearanceSettings />{groups.map(group => <section className="settings-group" key={group.title}><h2>{group.title}</h2><div className="white-card">{group.rows.map(row => <button className="settings-row" key={row.label} onClick={() => setSelected(row.label)}><span className="settings-icon"><UiIcon name={row.icon} /></span><span className="settings-label">{row.label}{row.desc && <small>{row.desc}</small>}</span>{row.value && <small>{row.value}</small>}<UiIcon name="chevron" /></button>)}</div></section>)}<button className="withdraw-button" onClick={() => setSelected("회원 탈퇴")}>회원 탈퇴</button></>}</div>
  </section>;
}
