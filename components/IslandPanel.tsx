...
import { contentReveal, islandSpring, islandWobble } from "@/lib/motion";

/**
 * 버튼에서 확장되는 검정 팝업. `layoutId`가 같은 버튼의 위치·크기에서
 * 스프링으로 커지고, 닫힐 때 다시 버튼으로 줄어든다.
 * 커지고 줄어드는 동안 `islandWobble`로 모서리와 비율이 버블처럼 출렁인다.
...
export default function IslandPanel({
  ...
  return (
    <motion.section
      ...
      animate={islandWobble.enter}
      exit={{ ...islandWobble.exit, transition: islandWobble.exitTransition }}
      transition={{ layout: islandSpring, default: islandWobble.enterTransition }}
      role="dialog"
      aria-modal="true"
      aria-label={title}
    >
